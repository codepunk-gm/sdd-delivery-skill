...
客户端会读取本仓库根目录的 `marketplace.json`，并从 `plugins/sdd-delivery` 加载插件。然后安装插件：
...
The client reads the root `marketplace.json` and loads the plugin from `plugins/sdd-delivery`. Then install the plugin:
