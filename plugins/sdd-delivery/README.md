...
如果已经安装过插件，拉取仓库更新后需要在 Codex 中重新安装或刷新插件，否则客户端可能仍读取旧版 skill：
...
- **企业 MCP 能力复用**：实现前查询企业内部工具、资源、组件、服务、API、SDK、脚手架、知识库或运维能力，避免重复手写。
- **团队 SQL 规范治理**：在 `.sdd-delivery/team-rules.json` 维护全局规范、项目覆盖和需求级例外，并可用命令持续更新。
...
python skills/sdd-delivery/scripts/manage_capabilities.py .sdd-delivery/login-rate-limit --project-root . --detect --plan
python skills/sdd-delivery/scripts/record_mcp_discovery.py .sdd-delivery/login-rate-limit --enable-capability --source "Codex MCP tools" --query-intent "查找企业内部可复用的账单查询 API、数据表格组件和权限校验 SDK"
python skills/sdd-delivery/scripts/setup_team_rules.py --root . --init
python skills/sdd-delivery/scripts/setup_team_rules.py --root . --sql-dialect postgresql --sql-schema billing --sql-migration-tool alembic
python skills/sdd-delivery/scripts/setup_team_rules.py --root . --sql-rule query.forbid_select_star=false
python skills/sdd-delivery/scripts/setup_team_rules.py --root . --sql-feature-exception "require_index_for_filter_columns::SPEC-3 export::一次性小表导出::tech lead::2026-09-01 前移除"
python skills/sdd-delivery/scripts/setup_team_rules.py --root . --validate
```

## 企业能力与团队规范

增强能力是可插拔的：默认由流程检测和用户确认决定是否启用，状态会记录在 checkpoint。

- 企业 MCP 能力复用：用于实现前发现企业内部已有能力，发现结果写入 `mcp-discovery.json` 和 `mcp-component-selection.md`。
- 团队规则：用于维护语言偏好、测试框架、代码原则、阈值和 SQL 规范。
- SQL 规范：支持 `global` 通用规则、`project_overrides` 项目特例、`feature_exceptions` 需求级临时例外。涉及 SQL、ORM、迁移、查询构造或数据访问层时，agent 应先读取这些规则再设计和实现。
